...
<style>
.field-error { color: #c0392b; font-size: 0.85em; margin-top: 2px; }
</style>
...
<c:if test="${not empty registered}">
  <div class="w3-panel w3-green w3-round">
    <p>Registration successful! You can now <a href="Login" class="menu">log in</a>.</p>
  </div>
</c:if>
...
<form id="registerForm" action="Register" method="POST" enctype="multipart/form-data">
  ...
  <div class="w3-section">
    ...
    <input class="w3-input w3-border w3-light-grey" type="text" id="name" name="name"
      ...
      title="2–60 letters, hyphens, or apostrophes."
      value="${user.name}" />
    ...
    <div class="w3-row">
      ...
      <div class="w3-rest">
        <input class="w3-input w3-border w3-light-grey" type="tel" id="phone" name="phone"
          pattern="\d{7,15}" title="7–15 digits, no spaces."
          value="${user.phone}" />
      </div>
    </div>
  ...
  <div class="w3-section">
    ...
    <input class="w3-input w3-border w3-light-grey" type="text" id="dni" name="dni"
      ...
      title="8 digits followed by a letter (e.g. 12345678A)."
      value="${user.dni}" />
    <c:if test="${not empty errors['dni']}">
      <span class="field-error">${errors['dni']}</span>
    </c:if>
  </div>

  <div class="w3-section">
    <label for="zip" class="w3-text-theme">ZIP Code *</label>
  ...
  <div class="w3-section">
    <label for="country" class="w3-text-theme">Country (ISO code) *</label>
    <input class="w3-input w3-border w3-light-grey" type="text" id="country" name="country"
      required pattern="[A-Za-z]{2}" maxlength="2"
      title="2-letter ISO code, e.g. ES, US, FR."
      value="${user.country}" />
  ...
  <div class="w3-section">
    ...
    <input class="w3-input w3-border w3-light-grey" type="text" id="username" name="username"
      ...
      title="4–30 characters: letters, digits, underscores."
      value="${user.username}" />
  ...
  <div class="w3-section">
    ...
    <input class="w3-input w3-border w3-light-grey" type="password" id="confirmPassword" name="confirmPassword"
      required title="Must match the password above." />
  ...
  <button type="submit" class="w3-button w3-theme w3-section w3-block">Register</button>
